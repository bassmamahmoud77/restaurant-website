    ...
    >
      <motion.div variants={staggerContainer(0.3, 1)} initial='hidden' animate={'show'}  className="container mx-auto" >
        <div className="flex justify-between items-center px-4 lg:px-0 relative text-white">
          <motion.div variants={fadeIn('down', 'tween', 1, 1.4)}  className={`${nav ? 'gap-y-0' : 'gap-y-2'} flex flex-col items-center justify-center w-12 h-12 p-3 order-2 lg:order-none cursor-pointer border-2 rounded-full`} onClick={() => setNav(!nav)}>
            <motion.div initial={{
              rotate: 0
            }}
          ...
          <motion.div variants={fadeIn('down', 'tween', 1.2, 1.4)} className="order-1 lg:order-none lg:ml-[11rem]">
            <a href="#">
              <img
                className={`${isActive ? "w-[90px] " : "w-[107px] h-[107px]"} `}
                src={LogoWhite}
                alt=""
              />
            </a>
